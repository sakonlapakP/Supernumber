...
@section('content')
          ...
          <table class="qdoc-items-table" data-qdoc-items>
            <thead>
              <tr>
                ...
                <th class="qdoc-col-price" data-qdoc-price-header>ราคา/หน่วย</th>
  ...
  <script>
    (() => {
      ...
      // ─── Mode 2 helper ───
      const isMode2 = () => vatMode === 'customer' && whtMode === 'company';

      const updatePriceHeader = () => {
        const header = document.querySelector('[data-qdoc-price-header]');
        if (header) header.textContent = isMode2() ? 'Net ที่อยากได้/หน่วย' : 'ราคา/หน่วย';
      };
      ...
      whtModeButtons.forEach(b => b.addEventListener('click', () => {
        ...
        updatePriceHeader();
        syncTotals();
        showStatus(isMode2() ? 'Mode 2: ลูกค้าจ่าย VAT, เรารับผิดชอบ WHT (กรอก Net ที่ต้องการ)' : (whtMode === 'company' ? 'คำนวณแบบเรารับผิดชอบ WHT' : 'คำนวณแบบลูกค้ารับผิดชอบ WHT'), 'info');
      }));
      ...
      vatModeButtons.forEach(b => b.addEventListener('click', () => {
        ...
        updatePriceHeader();
        syncTotals();
        showStatus(isMode2() ? 'Mode 2: ลูกค้าจ่าย VAT, เรารับผิดชอบ WHT (กรอก Net ที่ต้องการ)' : (vatMode === 'customer' ? 'คำนวณแบบลูกค้ารับผิดชอบ VAT' : 'คำนวณแบบเรารับผิดชอบ VAT'), 'info');
      }));
      ...
      const syncTotals = () => {
        ...
        getItemRows().forEach(row => {
          ...
          let amount;
          if (mode2 && whtRate > 0) {
            // Mode 2: price = desired NET per unit → back-calculate base
            const basePerUnit = roundMoney(price / (1 - whtRate / 100));
            amount = roundMoney(qty * basePerUnit);
          } else {
        ...
        let vatAmount, grandTotal, whtAmount, netToPay;
        if (mode2) {
          // Mode 2: afterDiscount IS the base; VAT added on top; WHT on base
          vatAmount = roundMoney(afterDiscount * vatRate / 100);
          grandTotal = roundMoney(afterDiscount + vatAmount);
          whtAmount = roundMoney(afterDiscount * whtRate / 100);
          netToPay = roundMoney(grandTotal - whtAmount);
        } else {
          const vatMultiplier = 1 + (vatRate / 100);
          const taxableAmount = vatMode === 'customer' && vatMultiplier > 0
            ? roundMoney(afterDiscount / vatMultiplier)
            : afterDiscount;
          vatAmount = vatMode === 'customer'
            ? roundMoney(afterDiscount - taxableAmount)
            : roundMoney(taxableAmount * vatRate / 100);
          grandTotal = vatMode === 'customer'
            ? roundMoney(afterDiscount)
            : roundMoney(afterDiscount + vatAmount);
          whtAmount = roundMoney(taxableAmount * whtRate / 100);
          netToPay = roundMoney(grandTotal - whtAmount);
        }
      ...
      const buildPayload = (docType) => {
        ...
        const items = getItemRows().map((row, i) => {
          ...
          const enteredPrice = parseNum(row.querySelector('[data-qdoc-item-price]')?.value);
          let unitPrice, amount;
          if (mode2 && whtRate > 0) {
            unitPrice = roundMoney(enteredPrice / (1 - whtRate / 100));
          ...
          subtotal = roundMoney(subtotal + amount);
          return { index: i + 1, description: desc, quantity: qty, quantity_display: fmtQty(qty), unit, unit_price: unitPrice, unit_price_display: fmtMoney(unitPrice), amount, amount_display: fmtMoney(amount) };
        });
        ...
        if (mode2) {
          ...
          vatAmount = vatMode === 'customer' ? roundMoney(afterDiscount - taxableAmount) : roundMoney(taxableAmount * vatRate / 100);
          grandTotal = vatMode === 'customer' ? roundMoney(afterDiscount) : roundMoney(afterDiscount + vatAmount);
          whtAmount = roundMoney(taxableAmount * whtRate / 100);
          netToPay = roundMoney(grandTotal - whtAmount);
        }
        ...
        return {
          ...
          calculator_mode: mode2 ? 'mode2-net-input' : (whtMode === 'company' ? 'post-vat-net' : 'normal'),
