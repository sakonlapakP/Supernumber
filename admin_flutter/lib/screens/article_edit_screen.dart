...
class _ArticleEditScreenState extends State<ArticleEditScreen> {
  ...
  void initState() {
    ...
    _metaDescController = TextEditingController(text: widget.article?.metaDescription ?? '');
    _keywordsController = TextEditingController(text: widget.article?.keywords ?? '');
    _lsiKeywordsController = TextEditingController(text: widget.article?.lsiKeywords ?? '');
    
    _htmlController = HtmlEditorController();
    _initialHtml = widget.article?.content ?? '';

    final guidelines = widget.article?.imageGuidelines;
    _promptLandscapeController = TextEditingController(text: guidelines?['landscape_prompt'] ?? '');
    _promptSquareController = TextEditingController(text: guidelines?['square_prompt'] ?? '');
  ...
  Future<void> _loadFullDetail() async {
    ...
    final fullArticle = await context.read<ArticleProvider>().fetchArticleDetail(widget.article!.id!);
  ...
  Future<void> _selectDateTime() async {
    ...
    if (pickedDate != null) {
      ...
      if (pickedTime != null) {
        setState(() {
          _publishedAt = DateTime(pickedDate.year, pickedDate.month, pickedDate.day, pickedTime.hour, pickedTime.minute);
        });
      }
    }
  }
  ...
  void _copyToClipboard(String text, String label) {
    if (text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('ไม่มีข้อความให้คัดลอก')),
      );
      return;
    }
  ...
  void _save() async {
    if (_formKey.currentState!.validate()) {
      ...
      final success = await context.read<ArticleProvider>().saveArticle(
        article,
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(backgroundColor: Color(0xFF1B8B6F), content: Text('บันทึกเรียบร้อย')),
        );
      }
    }
  }
          ...
          child: Form(
            ...
            child: ListView(
              ...
              children: [
                ...
                const Text('เนื้อหาบทความ', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF7488A8))),
                  ...
                  child: HtmlEditor(
                    ...
                    htmlToolbarOptions: const HtmlToolbarOptions(
                      ...
                      defaultToolbarButtons: [
                        ...
                        InsertButtons(video: false, audio: false, table: true, hr: true, otherFile: false),
                ...
                _buildTextField('Meta Description (สำหรับ Google)', _metaDescController, maxLines: 2),
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    IconButton(
                      icon: const Icon(Icons.copy_rounded, size: 20, color: Color(0xFF223A63)),
                      onPressed: () => _copyToClipboard(_promptLandscapeController.text, 'Landscape Prompt'),
                ...
                _buildTextField('Prompt สำหรับรูป 16:9', _promptLandscapeController, maxLines: 2),
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      icon: const Icon(Icons.copy_rounded, size: 20, color: Color(0xFF223A63)),
                      onPressed: () => _copyToClipboard(_promptSquareController.text, 'Square Prompt'),
                      tooltip: 'คัดลอก Prompt',
                    ),
                  ],
                ),
                ...
                _buildTextField('Prompt สำหรับรูป 1:1', _promptSquareController, maxLines: 2),
                ...
                ElevatedButton(onPressed: _save, child: const Text('บันทึกบทความ')),
  ...
  Widget _buildImagePicker({
    ...
    String? imageUrl;
    if (serverPath != null) {
      imageUrl = '${ApiService.dio.options.baseUrl.replaceAll('/api', '')}/storage/$serverPath';
    }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text('ยังไม่มีรูปภาพ', style: TextStyle(color: Colors.grey, fontSize: 12)),
        ],
      ),
    );
  }
  ...
  Widget _buildSectionTitle(String title) {
    return Padding(
      ...
      child: Text(
        title,
        style: GoogleFonts.kanit(fontWeight: FontWeight.bold, fontSize: 16, color: const Color(0xFF223A63)),
      ),
    );
  }
  ...
  Widget _buildTextField(String label, TextEditingController controller, {bool isBold = false, int maxLines = 1}) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF7488A8))),
  ...
  Widget _buildPublishToggle() {
    return Container(
      ...
      child: SwitchListTile(
        title: const Text('เผยแพร่แล้ว', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        value: _isPublished,
  ...
  Widget _buildDateTimePicker() {
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            const Icon(Icons.calendar_today_outlined, size: 20, color: Color(0xFF223A63)),
            const SizedBox(width: 12),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('เวลาเผยแพร่', style: TextStyle(fontSize: 12, color: Color(0xFF7488A8))),
                Text(
                  _publishedAt != null ? DateFormat('dd/MM/yyyy, HH:mm').format(_publishedAt!) : 'เผยแพร่ทันที',
                  style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1E2D45)),
                ),
              ],
            ),
            const Spacer(),
