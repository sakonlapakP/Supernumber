...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Add personal_access_tokens if it exists
        if (Schema::hasTable('personal_access_tokens')) {
            $tables['personal_access_tokens'] = ['last_used_at', 'expires_at', 'created_at', 'updated_at'];
        }

        foreach ($tables as $table => $columns) {
            $this->convertTableToTimestamp($table, $columns);
        }
    }

    private function convertTableToTimestamp(string $table, array $columns): void
    {
        $driver = DB::getDriverName();
        $columnsToProcess = [];

        // Identify which columns actually need conversion (are not yet timestamps)
        foreach ($columns as $column) {
            if (Schema::hasColumn($table, $column)) {
                $type = Schema::getColumnType($table, $column);
                // In some Laravel versions getColumnType might return 'datetime' or 'timestamp'
        ...
        if (empty($columnsToProcess)) {
            // Check if we have leftover _ts columns to cleanup (from a failed run)
        ...
        // 3. Handle Indexes (Drop them if they contain these columns)
        ...
        // Restore specific indexes if they don't exist
    ...
    {
        // Reversing this would mean going back to bigInteger Unix timestamps
    }
};
