...
class LikaySeatMap
{
    ...
    {
        return Cache::remember('likay_seats_map', 300, function (): array {
            $rowZones = self::rowZones();
            $seats = [];

            foreach (range(1, 16) as $number) {
                $seats['V_' . $number] = $rowZones['V'] ?? 'yellow';
            }

            foreach (range(1, 18) as $number) {
                $seats['W_' . $number] = $rowZones['W'] ?? 'yellow';
            }
    ...
    {
        Cache::forget('likay_seats_map');
        Cache::forget('likay_row_zones');
    }
    ...
    {
        return Cache::remember('likay_row_zones', 300, function (): array {
            return DB::table('likay_row_zones')
