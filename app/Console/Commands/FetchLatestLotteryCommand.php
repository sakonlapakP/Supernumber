...
class FetchLatestLotteryCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            if (!empty($prizeRows)) {
                // Replace prizes atomically so a failure mid-loop can't leave the
                // result with a partially-deleted/half-written prize set on display.
                \Illuminate\Support\Facades\DB::transaction(function () use ($result, $prizeRows): void {
    ...
    private function syncLotteryArticleCover(LotteryResult $result, Carbon $now, bool $wasAlreadyComplete = false): ?Article
    {
        ...
        if (!$article) {
            ...
            // 🛡️ Protection: only refresh the auto-generated title/content.
            // If an admin (or the auto-gen flow) has customised the body we keep
            // their edits — the live prize table on the page is rendered from the
            // LotteryResult relation, not from this content field, so the displayed
            // results stay correct either way.
            $currentContent = trim((string) $article->content);
            $isAutoGeneratedContent = $currentContent === ''
                || Str::startsWith($currentContent, 'รายงานผลสลากกินแบ่งรัฐบาล');

            if ($isAutoGeneratedContent) {
